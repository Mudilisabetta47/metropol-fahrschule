    ...
    >
      {isPopular && (
        <div className="absolute top-0 right-0 rounded-bl-xl gradient-primary px-3 py-1">
          <span className="text-[10px] font-bold uppercase tracking-wider text-primary-foreground">{t("common.popular")}</span>
        </div>
      )}
      ...
      <div className="p-5">
        ...
        {restItems.length > 0 && (
          <button onClick={() => setExpanded(!expanded)} className="mt-3 flex w-full items-center justify-center gap-1 rounded-lg bg-accent/50 py-2 text-xs font-semibold text-muted-foreground transition-colors hover:bg-accent hover:text-foreground">
            {expanded ? t("common.showLess") : t("common.moreItems", { count: restItems.length })}
      ...
      <div className="border-t border-border/50 bg-accent/20 px-5 py-3">
        <Button variant="ghost" size="sm" className="w-full text-xs font-bold text-primary hover:text-primary" asChild>
          <Link to="/kontakt">
            {t("common.inquireNow")} <ChevronRight className="ml-1 h-3.5 w-3.5" />
          </Link>
        </Button>
      </div>
    </motion.div>
    ...
    <div className="min-h-screen">
      ...
      <section className="relative min-h-[280px] md:min-h-[340px] flex items-end overflow-hidden">
        ...
        <div className="container relative z-10 mx-auto px-4 pb-8 pt-28">
          <motion.div initial={{ opacity: 0, y: 30, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
            <span className="mb-1 inline-block text-xs font-bold uppercase tracking-[0.2em] text-primary-foreground/50">{t("prices.subtitle")}</span>
            <h1 className="text-3xl font-extrabold text-primary-foreground font-display md:text-5xl">{t("prices.title")}</h1>
            <p className="mt-2 max-w-xl text-sm text-primary-foreground/50">{t("prices.heroDesc")}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          ...
          <Tabs defaultValue="hannover" className="w-full">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ type: "spring", stiffness: 300, damping: 25 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-10 rounded-2xl border border-border bg-card p-6 text-center shadow-card">
            <p className="text-xs text-muted-foreground">
              {t("prices.footerNote")}{" "}
              <Link to="/kontakt" className="text-primary font-medium hover:underline">{t("prices.footerContact")}</Link> {t("prices.footerContactEnd")}
            </p>
            <p className="mt-2 text-[11px] text-muted-foreground/60">{t("prices.paymentMethods")}</p>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 12, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }} transition={{ type: "spring", stiffness: 300, damping: 25, delay: 0.15 }} className="mt-6 text-center">
