...
const Locations = () => {
  ...
  const locations = [
    { name: "Hannover", addr: "Engelbosteler Damm 1, 30167 Hannover", desc: t("locations.hannoverDesc"), path: "/standorte/hannover" },
    { name: "Garbsen", addr: "Planetenring 25–27, 30823 Garbsen", desc: t("locations.garbsenDesc"), path: "/standorte/garbsen" },
    { name: "Bremen", addr: "Bahnhofsplatz 41, 28195 Bremen", desc: t("locations.bremenDesc"), path: "/standorte/bremen" },
  ];
      ...
      <section className="gradient-dark py-20 noise relative overflow-hidden">
        <div className="container relative z-10 mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <span className="mb-3 inline-block text-xs font-bold uppercase tracking-[0.2em] text-primary">{t("locations.subtitle")}</span>
            <h1 className="text-4xl font-extrabold text-primary-foreground font-display md:text-6xl">{t("locations.title")}</h1>
            <p className="mx-auto mt-4 max-w-2xl text-lg text-primary-foreground/50">{t("locations.heroText")}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24">
        <div className="container mx-auto px-4">
          <div className="grid gap-6 md:grid-cols-3">
            {locations.map((loc, i) => (
              <motion.div key={loc.name} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }}>
                <Link to={loc.path} className="group flex h-full flex-col rounded-3xl border border-border bg-card p-8 shadow-card transition-all duration-500 hover:shadow-card-hover hover:-translate-y-2">
